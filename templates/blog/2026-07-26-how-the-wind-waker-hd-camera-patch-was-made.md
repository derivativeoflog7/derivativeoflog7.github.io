...
In this article, I'll  recount how I created the [third person camera patches for the Wind Waker HD](https://github.com/derivativeoflog7/WWHD_3pCameraTweaks), as in how I found out what to patch in order to be able to play the game without constantly having to fight the camera controls that I just couldn't get used to.  
...
FP26 immediately caught my eye, as it was the only one whose sign would invert between the two locations, and I soon realized that if I manually changed it's value while holding the stick up, it would cause the camera to move towards a different angle, with positive values making the camera move above Link, and negative values below.
